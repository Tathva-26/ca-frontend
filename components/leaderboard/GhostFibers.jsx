...
const GhostFibers = ({
    ...
    `
      ...
      float grainHash(vec2 point) {
        ...
        float hash =
          52.9829189 *
          fract(dot(point, vec2(0.065, 0.005)));
        ...
        for (int index = 0; index < MAX_LAYERS; index++) {
          ...
          float lines =
            abs(
              sin(
                p.x *
                (uLineFrequency + fi * uLineSpacing) +
                sin(p.y * 3.0 + time)
              )
            );
    ...
    `
		...
		const program = new Program(gl, {
			...
			uniforms: {
				...
				uLayers: {
					value: Math.min(Math.max(Math.round(layers), 1), 10),
				},
				...
				uLineColor: {
					value: new Float32Array([...hexToRgb(lineColor)]),
				},

				uGlowColor: {
					value: new Float32Array([...hexToRgb(glowColor)]),
				},
			},
		})
	...
	useEffect(() => {
		...
		/*
		 * The actual WebGL context is stored on
		 * the canvas so this component remains
		...
		// Uniform updates are handled by the
		// initialization effect's current program.
	}, [])

	/*
	 * Recreate the renderer when dpr changes.
	 * Other props are applied through CSS/custom
	 * animation state below.
	 *
	 * To keep the component truly self-contained,
	 * the WebGL initialization above uses the
	 * initial prop values.
	 */
	...
	return (
		<div
			ref={containerRef}
			className={`fixed inset-0 w-screen h-screen overflow-hidden ${className}`.trim()}
		/>
	)
}
